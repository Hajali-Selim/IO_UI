...
def update_cline(metric3_c,unit3_c,region3_c):
    ...
    try:
        ...
        fig = px.line(dataset, x='year', y=metric3_c, color='country', labels={'x':'year', 'y':metric3_c, 'color':'country'}, markers=True, hover_name='country')
    fig.update_xaxes(tickvals= np.arange(1995,2023,3)).update_layout(font={'size':15}, height=700, width=1300, hoverlabel={'font_size':16}).update_traces(line={'width':4}, marker={'size':10}, hovertemplate='<br>year: %{x}<br>'+str(metric3_c)+': %{y:.2f}%')
    return fig
...
def update_sline(metric3_s,unit3_s):
    ...
    try:
        ...
        fig = px.line(dataset, x='year', y=metric3_s, color='sector', labels={'x':'year', 'y':metric3_s, 'color':'country'}, markers=True, hover_name='sector')
    fig.update_xaxes(tickvals=np.arange(1995,2023,3)).update_layout(font={'size':15}, height=700, width=1300, hoverlabel={'font_size':16}).update_traces(line={'width':4}, marker={'size':10}, hovertemplate='year: %{x}<br>'+str(metric3_s)+': %{y:.2f}%')
    return fig

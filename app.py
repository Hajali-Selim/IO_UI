...
app.layout = html.Div(children=[
    ...
    dbc.Accordion([
    	dbc.AccordionItem([
    		...
    		dbc.Row([dbc.Col(dcc.Markdown('**Select metric**', style={'textAlign':'right'}), width=2), dbc.Col(dcc.Dropdown(metrics_list, 'coal vulnerability', id='metric1'), width=3),
    		        dbc.Col(dcc.Markdown('**Decompose metric**', style={'textAlign':'right'}), width=2), dbc.Col(dbc.Checklist(id='type1', switch=True, value=['On'], options=[{'label':'', 'value':'On'}], inputStyle={'margin-right':'10px'}), width=4),]),
    		dbc.Row([dbc.Col(dcc.Markdown('**Select unit**', style={'textAlign':'right'}), width=2), dbc.Col(dcc.Dropdown(units_list[:-1], 'country', id='unit1'), width=3),
    		...
    		dbc.Row([dbc.Col(dcc.Markdown('**Select type of data**', style={'textAlign':'right'}), width=2),
    		        ...
    		        dbc.Row([
        		        ...
        		        dcc.RangeSlider(min=1995, max=2019, step=1, value=[2015,2019], marks={1995:'1995', 2019:'2019'}, tooltip={'placement':'bottom', 'always_visible':True}, id='range1'),
        		        ]), width=5)
    		        ]),
    		dbc.Row([dbc.Col(dcc.Markdown('**Select regions**', style={'textAlign':'right'}), width=2), dbc.Col(dcc.Checklist(regions_list, regions_list, id='group1_c', inline=True, inputStyle={'margin-top':'10px', 'margin-right':'5px', 'margin-left':'30px'}), width=6), ]),
    		dbc.Row([dbc.Col(dcc.Markdown('**Select sector groups**', style={'textAlign':'right'}), width=2), dbc.Col(dcc.Checklist(groups_list, groups_list, id='group1_s', inline=True, inputStyle={'margin-top':'10px', 'margin-right':'5px', 'margin-left':'30px'}), )
    		        ])], ), ]),
    	...
    	dbc.AccordionItem(
    	    ...
    	    dbc.Row([dbc.Col(dcc.Markdown('**Select sector**', style={'textAlign':'right'}), width=2),
                dbc.Col(dcc.Dropdown(sectors_list, 'Petroleum Refinery', id='sector4_c'), width=4)]),
			...
			dbc.Row([dbc.Col(dcc.Markdown('**Select marker size\n (vulnerability)**', style={'textAlign':'right'}), width=2),
			    dbc.Col(dmc.SegmentedControl(vulnerability_list, 'oil vulnerability', id='metric4i_c'), width=4)]),
			...
			dbc.Row([dbc.Col(dcc.Markdown('**Select marker size\n (vulnerability)**', style={'textAlign':'right'}), width=2),
			    dbc.Col(dmc.SegmentedControl(vulnerability_list, 'gas vulnerability', id='metric4i_s'))]),
		    dbc.Row([dbc.Col(dcc.Markdown('**Select marker color**', style={'textAlign':'right'}), width=2),
...
def update_histogram(metric1,unit1,group1_s,group1_c,year1,order1,type1,changes1,range1):
    ...
    if changes1 == 'range of years': # if looking at variations
        year0, year1 = range1
        dataset_previous, dataset = data_y[year0][metric1].reset_index(), data_y[year1].reset_index()[[unit1,metric1]]
        dataset[metric1] = dataset[metric1].subtract(dataset_previous[metric1])
    else:
        dataset = data_y[year1]
    dataset = dataset[dataset.group.isin(group1_s)&dataset.region.isin(group1_c)]
...
def update_sheatmap(segment2_s,metric2_s,group2_s,order2_s,unselect2_s):
    ...
    if segment2_s == 'Worldwide averages':
        ...
        for r_idx in range(len(regions_list)):
            nb_sectors = len(dataset.sector.unique())
            fig.add_trace(px.density_heatmap(dataset[regions_list[r_idx]], x='year', y='sector', z=metric2_s, nbinsx=NY, nbinsy=nb_sectors).data[0], row=1, col=r_idx+1).update_traces(hovertemplate='<b>%{y} ('+r+', %{x})</b><br>'+str(metric2_s)+': %{z:.2f}')
...
def update_cline(metric3_c,unit3_c,region3_c):
    ...
    fig.update_xaxes(tickvals= np.arange(1995,2019,3)).update_layout(font={'size':15}, height=700, width=1300, hoverlabel={'font_size':14}).update_traces(line={'width':4}, marker={'size':10}, hovertemplate='<b>%{customdata[0]}</b><br>year: %{x}<br>'+str(metric3_c)+': %{y:.2f}<extra></extra>')
    return fig

@callback(Output('lines3_s', 'figure'), [Input(s, 'value') for s in ['metric3_s','unit3_s','group3_s']])
def update_sline(metric3_s,unit3_s,group3_s):    
    ...
    try:
        ...
        fig = px.line(dataset, x='year', y=metric3_s, color='sector', labels={'x':'year', 'y':ylabel, 'color':'sector'}, markers=True, custom_data=['sector'])
    fig.update_xaxes(tickvals=np.arange(1995,2019,3)).update_layout(font={'size':15}, height=700, width=1300, hoverlabel={'font_size':14}).update_traces(line={'width':4}, marker={'size':10}, hovertemplate='<b>%{customdata[0]}</b><br>year: %{x}<br>'+str(metric3_s)+': %{y:.2f}<extra></extra>')
    return fig

@callback(Output('scatter4_c', 'figure'), [Input(s, 'value') for s in ['sector4_c', 'metric4x_c', 'metric4y_c', 'metric4i_c','year4_c','color4_c','group4_c']] + [Input('click4_c', 'data')])
def update_cscatter(sector4_c, metric4x_c,metric4y_c,metric4i_c,year4_c,color4_c,group4_c, click4_c): # SINGLE SECTOR select
...
@callback(Output('scatter4_s', 'figure'), [Input(s, 'value') for s in ['country4_s', 'metric4x_s', 'metric4y_s', 'metric4i_s', 'year4_s', 'color4_s', 'group4_s']] + [Input('click4_s','data')])
def update_sscatter(country4_s, metric4x_s, metric4y_s, metric4i_s, year4_s, color4_s, group4_s, click4_s): # SINGLE COUNTRY select
    ...
    try:
        fig = px.scatter(dataset, x=xlabel, y=ylabel, size=zlabel, color=color4_s, labels={'x':xlabel, 'y':ylabel}, hover_name='sector', opacity=.7, color_continuous_scale='Jet', custom_data=['sector',zlabel, xlabel,ylabel])
    except:
        fig = px.scatter(dataset, x=xlabel, y=ylabel, size=zlabel, color=color4_s, labels={'x':xlabel, 'y':ylabel}, hover_name='sector', opacity=.7, color_continuous_scale='Jet', custom_data=['sector',zlabel, xlabel,ylabel])
    fig.update_layout(width=900, height=650, font={'size':16}, coloraxis_colorbar={'title':'vulnerability (%)', 'orientation':'v'}, hoverlabel={'font_size':18}, legend={'orientation':'h', 'yanchor':'bottom', 'y':1.02, 'entrywidth':200, 'title':None}, clickmode='event+select').add_hline(y=1, line_width=3, line_dash='dash', line_color='red', opacity=.7).add_vline(x=1, line_width=3, line_dash='dash', line_color='red', opacity=.7).update_traces(hovertemplate='<br>'.join(['<b>%{customdata[0]}</b>', str(zlabel)+': %{customdata[1]:.2f}', str(xlabel)+': %{customdata[2]:.2f}', str(ylabel)+': %{customdata[3]:.2f}']))
    return fig
